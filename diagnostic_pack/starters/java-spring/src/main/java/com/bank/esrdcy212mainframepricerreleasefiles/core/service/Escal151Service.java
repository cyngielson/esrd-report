...
/**
 * ESCAL151 - COBOL program `ESCAL151` orchestrates a 1-step business flow. It emits 47 distinct return-code value(s) across 48 assignment site(s), driven by 1 state-machine flag group(s). Depends on 3 external copybook(s): RTCCPY, BILLCPY, WAGECPY.
 *
 * Risk: 48/100 (MEDIUM)
...
public class Escal151Service {
    ...
    private ReturnCodeResponse _1000ValidateBillElements() {
        ...
        // IF ((COMORBID-CWF-RETURN-CODE = SPACES) OR VALID-COMORBID-CWF-RETURN-CD       THEN NEXT SENTENCE): RETURN CODE PPS-RTC = 81
    ...
    /**
     * CALCULATION - L864-1105
     * Branches: 28, GOTO: 0, CALLs: 4
     */
    private void _2000CalculateBundledFactors() {
        ...
        // COBOL: COMPUTE H-BUN-BSA-FACTOR  ROUNDED = CM-BSA ** ((H-BUN-BSA - 1.87) / .1)
        this.hBunBsaFactorRounded = Math.pow(cmBsa, ((hBunBsa - 1.87) / .1));
        ...
        // COBOL: COMPUTE H-BUN-ADJUSTED-BASE-WAGE-AMT  ROUNDED = (H-BUN-BASE-WAGE-AMT * H-BUN-AGE-FACTOR) * (H-BUN-BSA-FACTOR * H-BUN-BMI-FACTOR) * (H-BUN-ONSET-FACTOR * H-BUN-COMORBID-MULTIPLIER) * (H-BUN-LOW-VOL-MULTIPLIER).
        this.hBunAdjustedBaseWageAmtRounded = (hBunBaseWageAmt * hBunAgeFactor) * (hBunBsaFactor * hBunBmiFactor) * (hBunOnsetFactor * hBunComorbidMultiplier) * (hBunLowVolMultiplier);
    ...
    /**
     * CALCULATION - L1106-1174
     * Branches: 11, GOTO: 0, CALLs: 1
     */
    ...
    private void _2500CalcOutlierFactors() {
        ...
        // COBOL: COMPUTE H-OUT-BSA-FACTOR  ROUNDED = SB-BSA ** ((H-OUT-BSA - 1.87) / .1)
        this.hOutBsaFactorRounded = Math.pow(sbBsa, ((hOutBsa - 1.87) / .1));
        ...
        // COBOL: COMPUTE H-OUT-PREDICTED-SERVICES-MAP  ROUNDED = (H-OUT-AGE-FACTOR * H-OUT-BSA-FACTOR * H-OUT-BMI-FACTOR * H-OUT-ONSET-FACTOR * H-OUT-COMORBID-MULTIPLIER * H-OUT-LOW-VOL-MULTIPLIER).
        this.hOutPredictedServicesMapRounded = (hOutAgeFactor * hOutBsaFactor * hOutBmiFactor * hOutOnsetFactor * hOutComorbidMultiplier * hOutLowVolMultiplier);
    ...
    private void _3000LowVolFullPpsPayment() {
        // COBOL: COMPUTE H-LV-BUN-ADJUST-BASE-WAGE-AMT  ROUNDED = (H-BUN-BASE-WAGE-AMT * H-BUN-AGE-FACTOR) * (H-BUN-BSA-FACTOR * H-BUN-BMI-FACTOR) * (H-BUN-ONSET-FACTOR * H-BUN-COMORBID-MULTIPLIER).
        this.hLvBunAdjustBaseWageAmtRounded = (hBunBaseWageAmt * hBunAgeFactor) * (hBunBsaFactor * hBunBmiFactor) * (hBunOnsetFactor * hBunComorbidMultiplier);
    ...
    private void _3100LowVolOutPpsPayment() {
        // COBOL: COMPUTE H-LV-OUT-PREDICT-SERVICES-MAP  ROUNDED = (H-OUT-AGE-FACTOR * H-OUT-BSA-FACTOR * H-OUT-BMI-FACTOR * H-OUT-ONSET-FACTOR * H-OUT-COMORBID-MULTIPLIER).
        this.hLvOutPredictServicesMapRounded = (hOutAgeFactor * hOutBsaFactor * hOutBmiFactor * hOutOnsetFactor * hOutComorbidMultiplier);
    ...
    /**
     * CALCULATION - L1577-1688
     * Branches: 11, GOTO: 0, CALLs: 0
     */
    private void _5000CalcCompRateFactors() {
        ...
        // COBOL: COMPUTE H-BSA-FACTOR  ROUNDED = CR-BSA ** ((H-BSA - 1.87) / .1)
        this.hBsaFactorRounded = Math.pow(crBsa, ((hBsa - 1.87) / .1));

        // COBOL: COMPUTE H-BMI  ROUNDED = (B-PATIENT-WGT / (B-PATIENT-HGT ** 2)) * 10000.
        this.hBmiRounded = (bPatientWgt / Math.pow((bPatientHgt, 2))) * 10000;

        // COBOL: COMPUTE H-WAGE-ADJ-PYMT-AMT ROUNDED = (((H-PAYMENT-RATE * NAT-LABOR-PCT) * COM-CBSA-W-INDEX) + (H-PAYMENT-RATE * NAT-NONLABOR-PCT)) * CBSA-BLEND-PCT.
        this.hWageAdjPymtAmtRounded = (((hPaymentRate * natLaborPct) * comCbsaWIndex) + (hPaymentRate * natNonlaborPct)) * cbsaBlendPct;

        // COBOL: COMPUTE H-PYMT-AMT ROUNDED = (H-WAGE-ADJ-PYMT-AMT * H-BMI-FACTOR * H-BSA-FACTOR * CASE-MIX-BDGT-NEUT-FACTOR * H-AGE-FACTOR * DRUG-ADDON).
        this.hPymtAmtRounded = (hWageAdjPymtAmt * hBmiFactor * hBsaFactor * caseMixBdgtNeutFactor * hAgeFactor * drugAddon);
