...
/**
 * ESCAL160 - COBOL program `ESCAL160` orchestrates a 1-step business flow. It emits 47 distinct return-code value(s) across 48 assignment site(s), driven by 1 state-machine flag group(s). Depends on 3 external copybook(s): RTCCPY, BILLCPY, WAGECPY.
 *
 * Risk: 46/100 (MEDIUM)
...
public class Escal160Service {
    ...
    private void _2000CalculateBundledFactors() {
        ...
        // COBOL: COMPUTE H-BUN-BSA-FACTOR  ROUNDED = CM-BSA ** ((H-BUN-BSA - 1.90) / .1)
        this.hBunBsaFactorRounded = Math.pow(cmBsa, ((hBunBsa - 1.90) / .1));
    ...
    private void _2500CalcOutlierFactors() {
        ...
        // COBOL: COMPUTE H-OUT-BSA-FACTOR  ROUNDED = SB-BSA ** ((H-OUT-BSA - 1.90) / .1)
        this.hOutBsaFactorRounded = Math.pow(sbBsa, ((hOutBsa - 1.90) / .1));
